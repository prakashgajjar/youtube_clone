...
const GroupWatchVideo = () => {
  const location = useLocation();
  const getLikedVideo = async () => {
    try {

      const responce = await axios.get('http://localhost:3000/watchlater', {
        headers: { 'Content-Type': 'application/json' },
        withCredentials: true,
      });
      console.log(responce.data.user.watchLater[0].channel.channelName);
      if (responce.status == 200) {
        setYourVideos(responce.data.user.watchLater)
      } else {
  ...
  return (
      ...
      <div className="h-[895px] mt-8 w-96 bg-gradient-to-b from-red-800  to-transparent flex justify-center items-center   rounded-t-2xl">
        <div className="flex flex-col justify-start  w-[312px] h-[839px]">
          ...
          <div>
            <h1 className="text-2xl font-bold  mt-5 font-sans">Watch later</h1>
          {yourVideos[0] && <h1 className="text-lg font-bold font-sans  mt-5">{yourVideos[0].channel.channelName}</h1> }
            <div className="mt-1">
            {
            yourVideos && <div className="flex gap-2 text-sm opacity-65 font-bold">
           { yourVideos[0] &&  <h1>{yourVideos[0].channel.videos.length}</h1>   }
              <h1>No views</h1>
             {yourVideos[0] &&  <h1>Updated {moment(yourVideos[0].createdAt).fromNow()}</h1>}
              </div>
            }
            </div>
          </div>
...
export default GroupWatchVideo