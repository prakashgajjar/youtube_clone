...
const VideoLike = async (req,res)=>{
    ...
    if(!video.likes.includes(req.user.id)){
      ...
      await User.findOneAndUpdate({_id:req.user.id},{$push:{likedVideos : video._id}})
    }
    else{
    ...
    return res.status(200).json({message : 'Video Liked or disliked Successfully' ,video: video});
} catch (error) {
